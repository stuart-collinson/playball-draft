...
export const generateMetadata = async ({ params }: PageProps): Promise<Metadata> => {
  ...
  return { title: `Form · ${LEAGUE_LABELS[league]}` }
}
...
const FormPage = async ({ params }: PageProps): Promise<JSX.Element> => {
  ...
  void getQueryClient().prefetchQuery(api.fpl.leagueDetails.queryOptions({ leagueId }))

  return (
    <HydrateClient>
      <h1 className="mb-6 text-xl font-bold text-foreground">
        {LEAGUE_LABELS[league as LeagueSlug]} Form — This Gameweek
      </h1>
