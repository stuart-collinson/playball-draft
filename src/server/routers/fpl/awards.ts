...
export const awardsProcedures = {
  awards: publicProcedure.input(leagueIdsInput).query(async ({ input }): Promise<AwardsData> => {
    ...
    const [allDetails, bootstrap, allTxData, allTradesData, allChoicesData] = await Promise.all([
      ...
      Promise.all(
        input.leagueIds.map((id) =>
          fetchFpl<DraftChoicesResponse>(FPL_ENDPOINTS.draftChoices(id), SERVER_TTL.DRAFT_CHOICES),
        ),
      ),
    ])
