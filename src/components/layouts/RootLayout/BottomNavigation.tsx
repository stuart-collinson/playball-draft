...
import { BarChart2, Home, LayoutGrid, Medal, Trophy } from "lucide-react";
...
const NAV_ITEMS: {
  ...
  { section: "awards", label: "Awards", icon: Medal },
...
export const BottomNavigation = (): JSX.Element => {
  ...
  const activeSection =
    NAV_ITEMS.find((item) => pathname.startsWith(`/${item.section}`))
      ?.section ?? "home";
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background/95 backdrop-blur-sm">
      <div className="mx-auto flex max-w-5xl items-stretch">
        {NAV_ITEMS.map(({ section, label, icon: Icon }) => {
          ...
          const href =
            section === "home" ? "/home" : `/${section}/${leagueSlug}`;
