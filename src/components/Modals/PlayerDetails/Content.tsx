...
import { buildTradeDrops, findOwnershipEnd, isProcessedTrade } from "@pbd/lib/fpl/ownership"
...
const PlayerDetailsContent = ({ player }: Props): JSX.Element => {
  ...
  // Only processed trades — offers that were rejected, withdrawn or vetoed
  // never moved a player and don't count as trades made.
  const myTrades = useMemo(() => {
    if (!tradesData || !entryId) return []
    return tradesData.trades.filter(
      (t) => isProcessedTrade(t) && (t.offered_entry === entryId || t.received_entry === entryId),
    )
  }, [tradesData, entryId])
