...
export type SplitFlapTextProps = HTMLAttributes<HTMLDivElement> & {
  words?: string[]
  text?: string
  flipDuration?: number
  stagger?: number
...
const isNamedCharset = (value: string): value is keyof typeof CHARSETS => value in CHARSETS

const styles = `
.split-flap-text{font-family:'SFMono-Regular','Roboto Mono','Cascadia Code','Liberation Mono',Menlo,monospace;font-size:var(--split-flap-font-size,52px);font-weight:760;line-height:1;letter-spacing:.035em;font-variant-numeric:tabular-nums;gap:var(--split-flap-gap,6px)}
...
const resolveCharset = (charset: SplitFlapTextProps["charset"]): string => {
  if (charset && isNamedCharset(charset)) return CHARSETS[charset]
