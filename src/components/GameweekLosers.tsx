...
export const GameweekLosers = (): JSX.Element => {
  ...
  const getLoserImage = (
    ...
    const sorted = [...data.standings].sort((a, b) => {
      ...
      const pointsDiff =
        gameweekPointsFor(a.event_total, points[a.league_entry]) -
        gameweekPointsFor(b.event_total, points[b.league_entry])
  ...
  const premImage = getLoserImage(premData, premGoals, premPoints)
  const champImage = getLoserImage(champData, champGoals, champPoints)
