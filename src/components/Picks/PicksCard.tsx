...
export const PicksCard = ({
  ...
  <div className="relative pt-4">
    ...
    <div className="flex h-full flex-col rounded-xl border border-border bg-card px-2.5 pb-3 pt-6 text-center">
      <p className="text-sm font-bold leading-snug text-foreground">{playerName}</p>
      ...
      <p className="text-[11px] font-medium leading-tight text-muted-foreground">
        {managerName}
        {wasAuto && <span className="ml-1 text-[9px] opacity-50">auto</span>}
      </p>

      <p className="mt-0.5 text-[10px] text-muted-foreground/60">Rd {round}</p>
    </div>
  </div>
)
